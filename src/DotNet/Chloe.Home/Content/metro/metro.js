$(function () {
    ...
    $container.on('mousewheel', function (event) {
        if ($layout.hasClass('horizontal')) {
            var direct1 = event.originalEvent.wheelDelta;//chrome¡¢IE
            var direct2 = event.originalEvent.detail;//FirFox
            var direct = direct1 || direct2;
            if (direct && !$container.is(":animated")) {
                if (direct > 0) {
